...
static FcStatus msufrendFunc(PpWord funcCode)
    ...
    if (TraceFrend)
        {
        LogOut("Func; %4o active=%d full=%d", funcCode,
...
static void msufrendIo(void)
    ...
    if (TraceFrend)
        {
        LogOut("IO; fcode=%o B data=%4o active=%d full=%d",
    ...
    default:
        if (TraceFrend >= LL_WARNING)
            {
            LogOut("FREND: Did not process I/O; prev func=%4o", pFrendInt->FrendState.fr_last_func_code);
            }
        break;
        }
    }
