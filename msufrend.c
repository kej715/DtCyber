...
void msuFrendInit(u8 eqNo, u8 unitNo, u8 channelNo, char *deviceName)
    ...
    if (TraceFrend)
        {
        msuFrendInitLog("msuFrendLog.txt", "Cy");
        }
    ...
    msuFrendInitInterface(FALSE);
    msuFrendCreateSocket();
    msuFrendInitWait();
...
static FcStatus msufrendFunc(PpWord funcCode)
    ...
    PpWord funcCodeMasked = 07400 & funcCode;

    pFrendInt->FrendState.fr_last_func_code = funcCodeMasked;

    if (TraceFrend)
        {
        msuFrendLog("Func; %4o active=%d full=%d", funcCode,
    ...
    case FcFEFDES:    /* DESELECT 6000 CHANNEL ADAPTER */
        ...
        if (FcFEFDES == funcCode)
            ...
            if (TraceFrend)
                {
                msuFrendLog("FREND: Got DESELECT");
                }
            }
...
static void msufrendIo(void)
    ...
    if (TraceFrend)
        {
        msuFrendLog("IO; fcode=%o B data=%4o active=%d full=%d",
    ...
    default:
        if (TraceFrend >= LL_WARNING)
            {
            msuFrendLog("FREND: Did not process I/O; prev func=%4o", pFrendInt->FrendState.fr_last_func_code);
            }
        break;
        }
    }
