...
static void tracePrintRma(FILE *fp, Cpu180Context *cpu, u64 pva);
...
void traceCpu170(Cpu170Context *cpu, u32 p, u8 opFm, u8 opI, u8 opJ, u8 opK, u32 opAddress)
    ...
    */
    if ((traceMask & TRACECPU(cpu, TraceCpu170)) == 0)
        {
        return;
        }
    ...
    */
    ...
    case RBA:
        ...
        fprintf(cpuF[cpu->id], "A%d=%06o\n", opJ, cpu->regA[opJ]);
        break;

    case RBAB:
        fprintf(cpuF[cpu->id], "B%d=%06o    ", opI, cpu->regB[opI]);
        fprintf(cpuF[cpu->id], "A%d=%06o    ", opJ, cpu->regA[opJ]);
        fprintf(cpuF[cpu->id], "B%d=%06o\n", opK, cpu->regB[opK]);
        break;
    ...
    default:
        fprintf(cpuF[cpu->id], "unsupported register set %d\n", decode[opFm].regSet);
        break;
        }
    }
...
void traceCpu180(Cpu180Context *cpu, u64 p, u8 opCode, u8 opI, u8 opJ, u8 opK, u16 opD, u16 opQ)
    ...
    */
    ...
    //
    //  If the instruction is a BDP instruction, print its descriptor(s)
    //
    if (entry->mode >= VCjkB2)
        {
        switch (entry->mode)
            {
        default:
        case VCjkB2:
            ...
            tracePrintRma(cpuF[cpu->id], cpu, cpu->srcDesc.pva);
            fputs("\n", cpuF[cpu->id]);
            p += 4;
            fprintf(cpuF[cpu->id], "%06d %x %03x %08x  ", traceSequenceNo, (u8)((p >> 44) & Mask4), (u16)((p >> 32) & Mask12), (u32)(p & Mask32));
            ...
            tracePrintRma(cpuF[cpu->id], cpu, cpu->dstDesc.pva);
            fputs("\n", cpuF[cpu->id]);
            break;
        case VCjkiDB1:
            p += 4;
...
/*--------------------------------------------------------------------------
**  Purpose:        Print the RMA for a PVA.
**
**  Parameters:     Name        Description.
...
static void tracePrintRma(FILE *fp, Cpu180Context *cpu, u64 pva)
    ...
    if (tracePvaToRma(cpu, pva, &rma))
        {
        fprintf(fp, " [" FMT32_08x "]", rma);
        }
    }
...
void traceCall(Cpu180Context *cpu, u64 pva)
    ...
    /*
    **  Bail out if no trace of call frame is requested.
    */
    ...
    for (i = 0; i <= 4; i++)
        ...
        tracePrintRma(cpuF[cpu->id], cpu, cpu->regA[i]);
        }
    fputs("\n", cpuF[cpu->id]);
    }
...
void traceCallFrame(Cpu180Context *cpu, u64 sfsa, char *label)
    ...
    */
    ...
    if (tracePvaToRma(cpu, sfsa, &rma) == FALSE)
        {
        return;
        }
...
/*--------------------------------------------------------------------------
**  Purpose:        Print backtrace of CYBER 180 stack frames.
**
**  Parameters:     Name        Description.
**                  cpu         Pointer to CYBER 180 CPU context
**                  maxDepth    Maximum number of stack frames to dump
**
**------------------------------------------------------------------------*/
void traceDumpStackFrames(Cpu180Context *cpu, u16 maxDepth)
    ...
    /*
    **  Bail out if not tracing CYBER 180 CPU.
    */
    if ((traceMask & TRACECPU(cpu, TraceCpu180)) == 0)
        {
        return;
    ...
    fprintf(cpuF[cpu->id], "%06d CYBER 180 Stack\n", traceSequenceNo);
    fprintf(cpuF[cpu->id], "%06d   Current   P " FMT64_012x "\n", traceSequenceNo, cpu->regP);
    fprintf(cpuF[cpu->id], "%06d            A0 " FMT64_012x "\n", traceSequenceNo, cpu->regA[0]);
    fprintf(cpuF[cpu->id], "%06d            A1 " FMT64_012x "\n", traceSequenceNo, cpu->regA[1]);
    fprintf(cpuF[cpu->id], "%06d            A2 " FMT64_012x "\n", traceSequenceNo, cpu->regA[2]);

    sfsa = cpu->regA[2];

    for (depth = 1; depth < maxDepth; depth++)
        {
        if ((sfsa & Mask3) != 0) break;
        if (tracePvaToRma(cpu, sfsa, &rma) == FALSE)
            {
            return;
            }
        ...
        for (i = 1; i < 4; i++)
            ...
            if (PageOf(pva, cpu) != pageNum)
                {
                if (tracePvaToRma(cpu, pva, &rma) == FALSE)
                    {
                    return;
                    }
        ...
        fprintf(cpuF[cpu->id], "\n%06d   Frame %2d  P " FMT64_012x "\n", traceSequenceNo, depth, cpMem[wordAddrs[0]] & Mask48);
        fprintf(cpuF[cpu->id], "%06d            A0 " FMT64_012x "\n", traceSequenceNo, cpMem[wordAddrs[1]] & Mask48);
        fprintf(cpuF[cpu->id], "%06d            A1 " FMT64_012x "\n", traceSequenceNo, cpMem[wordAddrs[2]] & Mask48);
        sfsa = cpMem[wordAddrs[3]] & Mask48;
        fprintf(cpuF[cpu->id], "%06d            A2 " FMT64_012x "\n", traceSequenceNo, sfsa);
        }
    }

/*--------------------------------------------------------------------------
**  Purpose:        Print a CYBER 170 register file.
**
**  Parameters:     Name        Description.
**                  cpu         Pointer to CPU context
**                  fp          Pointer to file stream
...
void tracePrint170Registers(Cpu170Context *cpu, FILE *fp)
    ...
    fprintf(fp, "RA     %07o  ", cpu->regRaCm);
    fprintf(fp, "A%d %06o  ", 1, cpu->regA[1]);
    fprintf(fp, "B%d %06o", 1, cpu->regB[1]);
    fputs("\n", fp);

    fprintf(fp, "FL     %07o  ", cpu->regFlCm);
    fprintf(fp, "A%d %06o  ", 2, cpu->regA[2]);
    fprintf(fp, "B%d %06o", 2, cpu->regB[2]);
    fputs("\n", fp);

    fprintf(fp, "RAE   %08o  ", cpu->regRaEcs);
    fprintf(fp, "A%d %06o  ", 3, cpu->regA[3]);
    fprintf(fp, "B%d %06o", 3, cpu->regB[3]);
    fputs("\n", fp);

    fprintf(fp, "FLE   %08o  ", cpu->regFlEcs);
    fprintf(fp, "A%d %06o  ", 4, cpu->regA[4]);
    fprintf(fp, "B%d %06o", 4, cpu->regB[4]);
    fputs("\n", fp);

    fprintf(fp, "EM/FL %08o  ", cpu->exitMode);
    fprintf(fp, "A%d %06o  ", 5, cpu->regA[5]);
    fprintf(fp, "B%d %06o", 5, cpu->regB[5]);
    fputs("\n", fp);

    fprintf(fp, "MA      %06o  ", cpu->regMa);
    fprintf(fp, "A%d %06o  ", 6, cpu->regA[6]);
    fprintf(fp, "B%d %06o", 6, cpu->regB[6]);
    fputs("\n", fp);

    fprintf(fp, "STOP         %d  ", cpu->isStopped ? 1 : 0);
    fprintf(fp, "A%d %06o  ", 7, cpu->regA[7]);
    fprintf(fp, "B%d %06o  ", 7, cpu->regB[7]);
    fputs("\n", fp);
    fprintf(fp, "ECOND       %02o  ", cpu->exitCondition);
    fputs("\n", fp);
    fprintf(fp, "MonitorFlag %s", cpu->isMonitorMode ? "TRUE" : "FALSE");
    fputs("\n\n", fp);
    ...
    for (i = 0; i < 8; i++)
        ...
        fprintf(fp, "X%d ", i);
        data = cpu->regX[i];
        fprintf(fp, "%04o %04o %04o %04o %04o   ",
...
void traceExchange170(Cpu170Context *cpu, u32 addr, char *title, bool force)
    ...
    */
    if ((traceMask & TRACECPU(cpu, TraceExchange)) == 0
        || ((traceMask & TRACECPU(cpu, TraceCpu170)) == 0 && force == FALSE))
        {
        return;
        }
    if (title != NULL)
    ...
    fputs("\n", cpuF[cpu->id]);
    tracePrint170Registers(cpu, cpuF[cpu->id]);
    fputs("\n", cpuF[cpu->id]);
    }

/*--------------------------------------------------------------------------
**  Purpose:        Print a CYBER 180 register file.
**
**  Parameters:     Name        Description.
**                  cpu         Pointer to CPU context
**                  fp          Pointer to file stream
...
void tracePrint180Registers(Cpu180Context *cpu, FILE *fp)
    ...
    tracePrintRma(fp, cpu, cpu->regP & Mask48);
    fputs("\n\n", fp);
    for (i = 0; i < 16; i++)
        {
        data = cpu->regX[i];
        fprintf(fp, "A%X ", i);
        tracePrintPva(fp, cpu->regA[i]);
        fprintf(fp,"   X%X %04x %04x %04x %04x\n", i,
    ...
    fputs("\n", fp);
    fprintf(fp, "VMID %04x  VMCL %04x   LPID %02x\n", cpu->regVmid, cpu->regVmcl, cpu->regLpid);
    fprintf(fp, " UMR %04x   MMR %04x  Flags %04x\n", cpu->regUmr, cpu->regMmr, cpu->regFlags);
    fprintf(fp, " UCR %04x   MCR %04x    MDF %04x\n", cpu->regUcr, cpu->regMcr, cpu->regMdf);
    fputs("\n", fp);
    fprintf(fp, " MPS %08x  SIT %08x\n", cpu->regMps, cpu->regSit);
    fprintf(fp, " JPS %08x  PIT %08x\n", cpu->regJps, cpu->regPit);
    fprintf(fp, "  BC %08x\n", cpu->regBc);
    fputs("\n", fp);
    fprintf(fp, " PTA %08x  STA %08x\n", cpu->regPta, cpu->regSta);
    fprintf(fp, " PTL %02x        STL %03x\n", cpu->regPtl, cpu->regStl);
    fprintf(fp, " PSM %02x\n", cpu->regPsm);
    fputs("\n", fp);
    fputs(" UTP ", fp);
    tracePrintPva(fp, cpu->regUtp);
    fputs("  TP ", fp);
    tracePrintPva(fp, cpu->regTp);
    fputs("\n", fp);
    fputs(" DLP ", fp);
    tracePrintPva(fp, cpu->regDlp);
    fprintf(fp, "  DI %02x\n", cpu->regDi);
    fprintf(fp, "                     DM %02x\n", cpu->regDm);
    fputs("\n", fp);
    fprintf(fp, " LRN %x\n", cpu->regLrn);
    ...
    fputs("\n", fp);
    fprintf(fp, " MDW " FMT64_016x "  \n", cpu->regMdw);
    fputs("\n", fp);
    fprintf(fp, "MonitorFlag  %s\n", cpu->isMonitorMode ? "TRUE" : "FALSE");
...
void traceExchange180(Cpu180Context *cpu, u32 addr, char *title)
    ...
    **  Bail out if no trace of exchange jumps is requested.
    */
    if ((traceMask & TRACECPU(cpu, TraceCpu180 | TraceExchange)) != TRACECPU(cpu, TraceCpu180 | TraceExchange))
        {
        return;
        }
    fprintf(cpuF[cpu->id], "\n%06d %s CYBER %s exchange package at %08x\n\n", traceSequenceNo, title, cpu->regVmid == 0 ? "180" : "170", addr);
    tracePrint180Registers(cpu, cpuF[cpu->id]);
    fputs("\n", cpuF[cpu->id]);
    }
...
void traceMemoryBlock(Cpu180Context *cpu, u64 pva, u16 length, char *title)
    ...
    if ((traceMask & TRACECPU(cpu, TraceCpu180 | TraceBlockOp)) != TRACECPU(cpu, TraceCpu180 | TraceBlockOp))
        {
        return;
        }
    if (title != NULL)
    ...
    while (length > 0)
        ...
        ci       = 0;
        shift    = 64;
        fprintf(cpuF[cpu->id], "    " FMT32_08x, byteAddr);
        while (byteAddr < rma)
            {
            fputs("   ", cpuF[cpu->id]);
            cBuf[ci++] = ' ';
...
void traceTrap(Cpu180Context *cpu)
    ...
    */
    ...
    tracePrintRma(cpuF[cpu->id], cpu, cpu->regP);
    fprintf(cpuF[cpu->id], "\n%06d                TP " FMT64_012x, traceSequenceNo, cpu->regTp);
    if (tracePvaToRma(cpu, cpu->regTp, &rma) == FALSE)
        {
        return;
        }
    ...
    if (isExt)
        {
        if (tracePvaToRma(cpu, cpu->regTp + 8, &rma))
            {
...
bool traceValidateStack(Cpu180Context *cpu, u64 sfsa, u16 maxDepth, char *label)
    ...
    */
    if ((traceMask & TRACECPU(cpu, TraceValidateStack)) == 0)
        {
        return TRUE;
        }
...
void traceMonitorCondition(Cpu180Context *cpu, MonitorCondition cond)
    ...
    if ((traceMask & TRACECPU(cpu, TraceConditions)) != 0 && ((1 << (15 - cond)) & traceMcrMask) != 0)
        ...
        tracePrintRma(cpuF[cpu->id], cpu, cpu->nextP);
        fputs("\n", cpuF[cpu->id]);
        }
    }
...
void tracePageInfo(Cpu180Context *cpu, u16 hash, u32 pageNum, u32 pageTableIdx, u64 spid)
    {
    if ((traceMask & TRACECPU(cpu, TracePva)) != 0)
        {
...
**                  condition registers or the UTP register
**
**  Parameters:     Name        Description.
**                  cpu         Pointer to CYBER 180 CPU context
**                  pva         PVA to translate
**                  rma         (out) The translated RMA, if no conditions detected
**
**  Returns:        TRUE if successful and rma is valid
**
**------------------------------------------------------------------------*/
bool tracePvaToRma(Cpu180Context *cpu, u64 pva, u32 *rma)
    ...
    savedMask   = traceMask;
    traceMask  &= ~TRACECPU(cpu, TracePva);
    savedUtp    = cpu->regUtp;
    isOk        = cpu180PvaToRma(cpu, pva, AccessModeNone, rma, &pti, &cond);
    cpu->regUtp = savedUtp;
    traceMask   = savedMask;
...
/*--------------------------------------------------------------------------
**  Purpose:        Trace a ring zero condition.
**
**  Parameters:     Name        Description.
**                  cpu         CPU context
**                  pva         PVA with ring 0
...
u8 traceDasmPpOp(PpWord w1, PpWord w2, char *str)
    ...
    if (isCyber180)
        {    
        decodeTable = (w1 & 0100000) != 0 ? ppDecode180 : ppDecode170;
        fmt = "%-4.4s ";
        }
    else
        {
