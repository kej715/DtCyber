...
1. To clone the DtCyber repo:
    ...
    ```  
2. Install packages needed for building and running:
    ```
    ...
    ```
3. Reboot (or at least log out and back in again) so X11 discovers the new fonts.

4. Select the makefile appropriate for your platform and do:
    ```
    cd DtCyber
...
2. Install XCode's commandline utilities:
    ```
    xcode-select --install
    ```
3. Download the DtCyber source repository.
    ```
    git clone <DtCyber repo of your choice>
    ```
4. Install [Homebrew](https://brew.sh) or your preferred package manager:
    ```
    /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
    ```
5. Install [XQuartz](https://www.xquartz.org/) and X11 development tools via Homebrew:
    ```
    brew install --cask xquartz
    brew install freetype pkg-config
    ```
6. Install the current LTS version of Node.js and npm, and link it 
...
Based on starting with clean default image of 32 or 64 bit Raspberry Pi OS with desktop.
    ...
    No obvious benefit seen with 64-bit OS version.
...
On most Pi you will need to edit the 'cyber.ini' file for the appropriate
...
1. Install unofficial build of NodeJS for Arm6, e.g.:
	...
	node -v
	```
2. Set the CFLAGS in the Makefile to do the best build for this CPU (this works well on almost all Linux machines)
	```
	CFLAGS  = -O3 -I. $(INCL) $(EXTRACFLAGS) -std=gnu99 -march=native -mcpu=native -mtune=native
	```

ARM7: Raspberry Pi 2 Model B (Quad-core ARM Cortex-A7)
...
# Building DtCyber on Windows

To build *dtcyber* on Windows:

1. Install *Microsoft Visual Studio*, if your Windows system does not have it already.

2. Install [Git Large File Storage](https://git-lfs.com), if your Windows system does not have it already.

3. Install [Node.js](https://nodejs.org), if your Windows system does not have it already.

4. Launch *Visual Studio* and use its *git* menu to clone a DtCyber repo (e.g., https://github.com/kej715/DtCyber)

5. Open the solution file (*DtCyber.sln*) in Visual Studio and build the project defined by it.

6. Use Node.js to install the automation tools and helper applications that support running
and maintaining DtCyber. Do this by executing the following sequence of commands from the NOS2.8.7 subdirectory of the DtCyber repo cloned in step 4.
	```
	cd automation
	npm install
	cd ..\webterm
