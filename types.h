...
typedef struct
    ...
    bool   isDump;                      /* TRUE if PP is in dump mode */
    bool   isLoad;                      /* TRUE if PP is in load mode */
...
typedef struct
    ...
    u32             pageTableLimit;       /* address+1 of last entry in page table */
...
#define PR_HALT       0x10                /*   halt request                             */
    ...
    u8              opDm;                 /* debug mask of current instruction */
    bool            opDebug;              /* TRUE if debugging is enabled on current instruction */
