 ...
 */
void *FrendMapMemoryWindows(char *lpMappingName, DWORD dwNumberOfBytesToMap)
    {
    LPVOID addr = 0;
    ...
    //   dwGetVer = GetVersion();    We don't support Windows 95
 ...
 *             else NULL if failure.
 */
void *FrendMapMemoryLinux(char *lpMappingName, int NumberOfBytesToMap)
    {
    void *pStart    = NULL;
...
int CreateSockToFrend()
    ...
    if (retval)
        {
        LogOut("==**CreateSockToFrend got error %d", retval);
        }
...
int SendToFrend(Byte8 *buf, int len)
    ...
    if (retval)
        {
        LogOut("==**SendToFrend got error %d", retval);
        }
...
void ReplyToCyber()
    ...
    if (retval)
        {
        LogOut("==**ReplyToCyber got error %d", retval);
        }
#endif
    }
