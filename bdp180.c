...
static u16 maxBdpOpLengths[16] =
    ...
    19,  // Type 12: Translated Packed Decimal Signed
    19,  // Type 13: Translated Packed Decimal Signed Leading Slack Digit

    8,   // Type 14: Translated Binary Unsigned
    8    // Type 15: Translated Binary Signed
    };

//
// This table is used by BDP data types 12 - 15 to translate data _from_ memory.
// See Table 2.3-1 of MIGDS rev T.
//
static u8 xlateFromMemory[256] =
    {
    0x00,0x01,0x02,0x03,0x37,0x2d,0x2e,0x2f,0x16,0x05,0x25,0x0b,0x0c,0x0d,0x0e,0x0f,
    0x10,0x11,0x12,0x13,0x3c,0x3d,0x32,0x26,0x18,0x19,0x3f,0x27,0x1c,0x1d,0x1e,0x1f,
    0x40,0x4f,0x7f,0x78,0x5b,0x6c,0x50,0x7d,0x4d,0x5d,0x5c,0x4e,0x68,0x60,0x48,0x61,
...
//
// This table is used by BDP data types 12 - 15 to translate data _to_ memory.
// See Table 2.3-2 of MIGDS rev T.
//
static u8 xlateToMemory[256] =
    {
...
bool bdp180DecodeOperand(Cpu180Context *ctx, BdpDescriptor *desc, BdpOperand *operand)
    ...
    if (bdp180HasValidLength(desc) == FALSE)
        {
    ...
    if (type >= 12)
        ...
        for (i = 0; i < desc->length; i++)
            {
            buffer[i] = xlateFromMemory[buffer[i]];
            }
        }
    switch (type)
        {
    case 12: // Translated Packed Decimal Signed
        type = 2;
        break;
    case 13: // Translated Decimal Signed Leading Slack Digit
        type = 3;
        break;
    case 14: // Translated Binary Unsigned
        type = 10;
        break;
    case 15: // Translated Binary Signed
        type = 11;
        break;
    default:
        break;
    ...
    *isTruncated = FALSE;
    ...
    case 1:  // Packed Decimal No Sign Leading Slack Digit
        ...
        *isTruncated = IsLongZero(operand->value) == FALSE;
        if (type == 1 && desc->length > 0 && (buffer[0] & 0xf0) != 0)
            {
            *isTruncated = TRUE;
            buffer[0]   &= 0x0f;
            }
    ...
    case 3:  // Packed Decimal Signed Leading Slack Digit
        ...
        *isTruncated = IsLongZero(operand->value) == FALSE;
        if (type == 3 && desc->length > 0 && (buffer[0] & 0xf0) != 0)
            {
            *isTruncated = TRUE;
            buffer[0]   &= 0x0f;
            }
    ...
    case 11: // Binary Signed
        ...
        while (i > 0)
            ...
            if (operand->sign)
                {
                operand->value[3] |= 0xff00000000000000;
                }
            }
        if (operand->sign)
            {
            *isTruncated = (type == 11 && (buffer[i] & 0x80) == 0)
                        ...
                        || operand->value[0] != 0xffffffffffffffff;
            }
        else
            {
            *isTruncated = (type == 11 && (buffer[i] & 0x80) != 0)
    ...
    if (*isTruncated == FALSE || inhOnTrunc == FALSE)
        ...
        if (desc->type >= 12)
            ...
            for (i = 0; i < desc->length; i++)
                {
                buffer[i] = xlateToMemory[buffer[i]];
                }
            }
...
/*--------------------------------------------------------------------------
**  Purpose:        Determine whether a BDP descriptor has a valid length
**                  for its type
**
**  Parameters:     Name        Description.
**                  desc        pointer to BDP destination descriptor
...
bool bdp180HasValidLength(BdpDescriptor *desc)
    {
    return desc->length <= maxBdpOpLengths[desc->type];
    }
