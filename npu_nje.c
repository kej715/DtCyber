        ...
        *dp++ = srcb; // SRCB
        ...
        if (rcb == RCB_GCR)
            ...
            while (bp < recLimit)
                {
                nBytes = recLimit - bp;
                if (nBytes > 63) nBytes = 63;
                *dp++ = 0xc0 + nBytes; // SCB
                while (nBytes-- > 0) *dp++ = *bp++;
                }
...
#endif
    tcbp = npuNjeFindTcb(pcbp);
    if (tcbp != NULL) npuSvmDiscRequestTerminal(tcbp);
...
static u8 *npuNjeCollectBlock(Pcb *pcbp, u8 *dp, u8 *limit, bool *isComplete, int *status)
    ...
    */
    ...
    if (currentBlockSize < TtbLength) return dp;
    /*
    ...
    */
    if (currentBlockSize >= njeBlockSize)
        ...
        while (sp + TtrLength < ibLimit)
            ...
            sp += TtrLength;
            while (sp < ibLimit && recLen-- > 0) *ibp++ = *sp++;
            }
...
static void npuNjeTransmitQueuedBlocks(Pcb *pcbp)
    ...
    if (tcbp != NULL)
        ...
        while (pcbp->controls.nje.uplineBlockLimit > 0
            ...
            if (tcbp->uplineBsn >= 8) tcbp->uplineBsn = 1;
                ...
                *obp++ = *srcb;
                ...
                while (ibp < ibLimit && *ibp != 0x00)
                    ...
                    case 0x80: // compressed string
                        ...
                        if ((scb & 0x20) == 0x20)
                            {
                            while (len-- > 0 && recLen++ < 255) *obp++ = *ibp;
                            ibp += 1;
                            }
                        else
                            {
                            while (len-- > 0 && recLen++ < 255) *obp++ = EbcdicBlank;
                            }
                        break;
                    case 0xc0: // non-compressed string
                        len = scb & 0x3f;
