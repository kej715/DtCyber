...
.then(() => term.expect([{ re: /\?\? / }]))
.then(() => term.send("L/USER(REXEC,/\r"))
.then(() => term.expect([{ re: /\?\? / }]))
.then(() => term.say("Located USER(REXEC ..."))
.then(() => term.send("D\r"))
.then(() => term.expect([{ re: /\?\? / }]))
.then(() => term.send("IB1\r"))
.then(() => term.expect([{ re: /\? / }]))
.then(() => term.send(`USER(REXEC,${utilities.getPropertyValue(customProps,"PASSWORDS","REXEC","REXECX")})\r`))
.then(() => term.expect([{ re: /\?\? / }]))
.then(() => term.say("Updated password of REXEC ..."))
